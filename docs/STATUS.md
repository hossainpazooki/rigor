...
| all 8 commands, `session-start`, the 10 pre-existing check scripts | commands / hook / gates | provisional (`check-citation-fidelity` carries a logged limit; `check-runlog` gained `resolveSupersession` as a shared export 2026-08-22; **`check-dispatch` helped 2026-08-22** — first live catch of a fabricated worker receipt, left red on the record) |

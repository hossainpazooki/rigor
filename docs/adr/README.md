...
| [0009](0009-agent-output-snapshots.md) | Snapshots ledger — `docs/snapshots/` verbatim agent-output exhibits (claimant-report by construction, no orphans, load-bearing threshold, handoff sole writer) as the kit's third folder | **Proposed** 2026-08-02 | Nothing built — no folder, no `check-snapshots`, no skill edits | Three pending decisions (seed-vs-forward for the motivating tic skeptic report; run-log citations; gate placement) await the operator |

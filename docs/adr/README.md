...
| [0006](0006-silent-tier-collapse.md) | Silent tier collapse — an unpinned `agent()` call inherits the session model, so a fan-out can look like a Sonnet-5 swarm while Fable silently does all the work; mechanize the build-tier pin check | **Proposed** 2026-07-15 — *not ratified* | Nothing. Zero surface, by design — the ADR is the artifact | ① Operator must ratify the 3 resolutions. ② Load-bearing tic-build finding needs independent re-verification (carried from prior-session memory, not re-checked this session). ③ Needs `check-fanout.mjs`'s new warning verified non-vacuous (red on a deliberately unpinned script) before it counts as built |
